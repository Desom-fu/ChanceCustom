...
dictSLMap = {
    'matchTypeList': ['完全匹配', '模糊匹配', '前缀匹配', '正则匹配'],
    'matchTypeList_loadMap': {'full': '完全匹配', 'contain': '模糊匹配', 'perfix': '前缀匹配', 'reg': '正则匹配'},
    'matchTypeList_saveMap': {'完全匹配': 'full', '模糊匹配': 'contain', '前缀匹配': 'perfix', '正则匹配': 'reg'},
    'matchPlaceList': ['群聊触发', '私聊触发', '群/私聊触发'],
    'matchPlaceList_loadMap': {'1': '群聊触发', '2': '私聊触发', '3': '群/私聊触发'},
    'matchPlaceList_saveMap': {'群聊触发': '1', '私聊触发': '2', '群/私聊触发': '3'},
}
...
class BrowserChoiceDialog(object):
    def __init__(self, parent, title, message, url):
        ...
        msg_label = tkinter.Label(self.dialog, text=message, bg='#00A0EA', fg='#FFFFFF', font=('等线', 11))
        msg_label.pack(pady=(20, 5))

        url_label = tkinter.Label(self.dialog, text=url, bg='#00A0EA', fg='#BBE9FF', font=('等线', 9), wraplength=320)
...
class ConfigUI(object):
    ...
    def start(self):
        ...
        self.UIObject['root_hash'].current(self.UIData['hash_list'].index(self.UIData['hash_default_key']))
        ...
        self.tree_UI_Button_init(
            name='root_Button_PACK', text='打包...', command=lambda: self.ccpk_pack(), x=330, y=15, width=80, height=34
        )

        self.tree_UI_Button_init(
            name='root_Button_SAVE', text='保存', command=lambda: self.tree_save(), x=420, y=15, width=80, height=34
        )
        ...
        self.UIObject['root_hint_label'].place(x=130, y=60, width=150, height=20)
    ...
    def init_notebook(self):
        ...
        self.UIData['style'].map(
            'TNotebook.Tab',
            background=[('selected', self.UIConfig['color_004']), ('!selected', self.UIConfig['color_003'])],
            foreground=[('selected', self.UIConfig['color_003']), ('!selected', self.UIConfig['color_004'])],
        )
    ...
    def tree_init(self):
        ...
        self.UIObject['frame_key_root'].grid(
            row=0, column=0, sticky='nsew', rowspan=1, columnspan=1, padx=(0, 0), pady=(0, 0), ipadx=0, ipady=0
        )
        ...
        self.UIObject['tree'].grid(
            row=0, column=0, sticky='nswe', rowspan=1, columnspan=1, padx=(0, 0), pady=(0, 0), ipadx=0, ipady=0
        )
        self.UIObject['tree'].bind('<Button-3>', lambda x: self.tree_rightKey(x))
        self.UIObject['tree_yscroll'] = ttk.Scrollbar(
            self.UIObject['frame_key_root'], orient='vertical', command=self.UIObject['tree'].yview
        )
        self.UIObject['tree'].configure(yscrollcommand=self.UIObject['tree_yscroll'].set)
        self.UIObject['tree_yscroll'].grid(
            row=0, column=1, sticky='nse', rowspan=1, columnspan=1, padx=(0, 0), pady=(0, 0), ipadx=0, ipady=0
        )
        ...
        self.UIObject['tree_ccpk_yscroll'] = ttk.Scrollbar(
            self.UIObject['frame_ccpk_root'], orient='vertical', command=self.UIObject['tree_ccpk'].yview
        )
        self.UIObject['tree_ccpk'].configure(yscrollcommand=self.UIObject['tree_ccpk_yscroll'].set)
    ...
    def tree_UI_singal_Label_Entry_init(
        self, obj_root: str, obj_name: str, title: str, str_name: str, count: int, mode: str = 'NONE'
    ):
        tmp_span_len = 4
        tmp_both_width_len = 400
        tmp_label_height_len = 20
        tmp_entry_height_len = 20
        self.tree_UI_singal_Label_init(obj_root, obj_name, title)
        self.UIData[str_name] = tkinter.StringVar()
        self.tree_UI_singal_Entry_init(obj_root, obj_name, str_name, mode)
    ...
    def tree_UI_singal_Entry_init(self, obj_root, obj_name, str_name, mode='NONE'):
        self.UIObject[obj_name + '=Entry'] = tkinter.Entry(self.UIObject[obj_root], textvariable=self.UIData[str_name])
        self.UIObject[obj_name + '=Entry'].configure(bg=self.UIConfig['color_004'], fg=self.UIConfig['color_005'], bd=0)
        if mode == 'SAFE':
    ...
    def tree_UI_Combobox_init(self, obj_root, obj_name, str_name, x, y, width_t, width, height, action, title=''):
        ...
        self.UIObject[obj_name].bind(
            '<<ComboboxSelected>>', lambda x: self.tree_UI_Combobox_ComboboxSelected(x, action, obj_name)
        )
    ...
    def tree_load(self):
        ...
        it_list_tmp_dictCustomData = ChanceCustom.load.getCustomDataSortKeyList(
            data=tmp_dictCustomData['data'][tmp_hashSelection], reverse=True
        )
        ...
        try:
            self.UIData['default_reply_day_StringVar'].set(
                tmp_dictCustomData['defaultVar'][tmp_hashSelection]['一天上限']
            )
            self.UIData['default_reply_week_StringVar'].set(
                tmp_dictCustomData['defaultVar'][tmp_hashSelection]['一周上限']
            )
            self.UIData['default_reply_month_StringVar'].set(
                tmp_dictCustomData['defaultVar'][tmp_hashSelection]['一月上限']
            )
            self.UIData['default_reply_once_StringVar'].set(
                tmp_dictCustomData['defaultVar'][tmp_hashSelection]['一次间隔']
            )
            self.UIData['default_reply_reply_StringVar'].set(
                tmp_dictCustomData['defaultVar'][tmp_hashSelection]['回复间隔']
            )
    ...
    def tree_save(self):
        try:
            tmp_dictCustomData = ChanceCustom.load.dictCustomData
            tmp_hashSelection = self.UIData['hash_now']
            tmp_dictCustomData['defaultVar'][tmp_hashSelection]['一天上限'] = self.UIData[
                'default_reply_day_StringVar'
            ].get()
            tmp_dictCustomData['defaultVar'][tmp_hashSelection]['一周上限'] = self.UIData[
                'default_reply_week_StringVar'
            ].get()
            tmp_dictCustomData['defaultVar'][tmp_hashSelection]['一月上限'] = self.UIData[
                'default_reply_month_StringVar'
            ].get()
            tmp_dictCustomData['defaultVar'][tmp_hashSelection]['一次间隔'] = self.UIData[
                'default_reply_once_StringVar'
            ].get()
            tmp_dictCustomData['defaultVar'][tmp_hashSelection]['回复间隔'] = self.UIData[
                'default_reply_reply_StringVar'
            ].get()
                            ...
                            if 'type' in ccpk_data and ccpk_data['type'] == 'ccpk':
                                if 'dataVersion' in ccpk_data and ccpk_data['dataVersion'] == 2:
                                    for key_this in ccpk_data['data']:
                                        tmp_dictCustomData['data'][tmp_hashSelection][key_this] = ccpk_data['data'][
                                            key_this
                                        ]
                                    tmp_dictCustomData['ccpkList'][tmp_hashSelection][ccpk_data['info']['name']] = (
                                        ccpk_data
                                    )
    ...
    def ccpk_pack(self):
        ...
        PackUpUI(action='create', key=None, bot_hash=bot_hash_now, edit_commit_callback=None, root=self).start()
    ...
    def open_forum(self):
        ...
        try:
            # 创建三选一对话框
            dialog = BrowserChoiceDialog(self.UIObject['root'], '程心自定义使用教学', '请选择打开方式：', url)
    ...
    def tree_rightKey(self, event, name='key'):
        if name == 'key':
            ...
            self.UIObject['tree_ccpk_rightkey_menu'].add_command(
                label='卸载', command=lambda: self.ccpk_remove(flagDelete=True)
            )
            self.UIObject['tree_ccpk_rightkey_menu'].add_command(
                label='解绑(不删关键词)', command=lambda: self.ccpk_remove(flagDelete=False)
            )
            self.UIObject['tree_ccpk_rightkey_menu'].add_command(
                label='重新安装', command=lambda: self.ccpk_reinstall()
            )
            ...
            if key_now in tmp_dictCustomData['ccpkList'][bot_hash_now]:
                for key_this in tmp_dictCustomData['ccpkList'][bot_hash_now][key_now]['data']:
                    if key_this not in tmp_dictCustomData['data'][bot_hash_now]:
                        tmp_dictCustomData['data'][bot_hash_now][key_this] = tmp_dictCustomData['ccpkList'][
                            bot_hash_now
                        ][key_now]['data'][key_this]
            self.tree_save()
            self.tree_load()
        ...
        if action == 'delete':
            ...
            if not self.UIData['flag_open']:
                ...
                edit_action = TreeEditUI(
                    action=action, key=key_now, bot_hash=bot_hash_now, edit_commit_callback=None, root=self
                )
                edit_action.start()
...
class TreeEditUI(object):
    ...
    def start(self):
        ...
        self.UIObject['edit_root_value_yscroll'] = ttk.Scrollbar(
            self.UIObject['edit_root'], orient='vertical', command=self.UIObject['edit_root_value'].yview
        )
        self.UIObject['edit_root_value_yscroll'].place(x=550 - 18 - 15, y=15 + 45 * 3, width=18, height=30 * 8)
        self.UIObject['edit_root_value'].configure(yscrollcommand=self.UIObject['edit_root_value_yscroll'].set)
    ...
    def tree_edit_UI_Text_init(
        ...
        self.UIObject[obj_name + '=Label'].place(x=x - width_t, y=y, width=width_t, height=height)
        self.UIData[str_name] = tkinter.StringVar()
        self.UIObject[obj_name] = tkinter.Text(self.UIObject[obj_root], wrap=tkinter.WORD)
        self.UIObject[obj_name].configure(bg=self.UIConfig['color_004'], fg=self.UIConfig['color_005'], bd=0)
        self.UIObject[obj_name].place(x=x, y=y, width=width, height=height)
    ...
    def tree_edit_data_init(self):
        ...
        if self.action == 'create':
            ...
            self.UIObject['edit_root_matchType'].current(
                dictSLMap['matchTypeList'].index(dictSLMap['matchTypeList_loadMap'][tmp_data_this['matchType']])
            )
            self.UIObject['edit_root_matchPlace'].current(
                dictSLMap['matchPlaceList'].index(dictSLMap['matchPlaceList_loadMap'][tmp_data_this['matchPlace']])
            )
...
class PackUpUI(object):
    ...
    def start(self):
        ...
        self.UIObject['label_info'].place(x=15, y=15, width=600 - 15 * 2, height=24)

        self.UIObject['listbox_L_StringVar'] = tkinter.StringVar()
        self.UIObject['listbox_L'] = tkinter.Listbox(self.UIObject['edit_root'])
        self.UIObject['listbox_L'].configure(listvariable=self.UIObject['listbox_L_StringVar'])
        self.UIObject['listbox_L'].place(x=45, y=45, width=250 - 18, height=200)
        self.UIObject['listbox_L_yscroll'] = ttk.Scrollbar(
            self.UIObject['edit_root'], orient='vertical', command=self.UIObject['listbox_L'].yview
        )
        self.UIObject['listbox_L_yscroll'].place(x=45 + 250 - 18, y=45, width=18, height=200)
        self.UIObject['listbox_L'].configure(yscrollcommand=self.UIObject['listbox_L_yscroll'].set)
        self.UIObject['listbox_L'].bind(
            '<<ListboxSelect>>', lambda x: self.listbox_selected(x, 'listbox_L', 'listbox_R')
        )

        self.UIObject['listbox_R_StringVar'] = tkinter.StringVar()
        self.UIObject['listbox_R'] = tkinter.Listbox(self.UIObject['edit_root'])
        self.UIObject['listbox_R'].configure(listvariable=self.UIObject['listbox_R_StringVar'])
        self.UIObject['listbox_R'].place(x=600 - (250 + 45), y=45, width=250 - 18, height=200)
        self.UIObject['listbox_R_yscroll'] = ttk.Scrollbar(
            self.UIObject['edit_root'], orient='vertical', command=self.UIObject['listbox_R'].yview
        )
        self.UIObject['listbox_R_yscroll'].place(x=600 - 45 - 18, y=45, width=18, height=200)
        self.UIObject['listbox_R'].configure(yscrollcommand=self.UIObject['listbox_R_yscroll'].set)
        self.UIObject['listbox_R'].bind(
            '<<ListboxSelect>>', lambda x: self.listbox_selected(x, 'listbox_R', 'listbox_L')
        )
    ...
    def pack_save(self):
        ...
        ccpk_data = {
            'type': 'ccpk',
            'dataVersion': ChanceCustom.load.dataVersion,
            'info': {'name': ccpk_name, 'author': ccpk_author, 'version': ccpk_version, 'info': ccpk_info},
            'data': {},
        }
