    ...
    def flowInputFun(valDict):
        def flowInput_f(matched: 're.Match|dict'):
            ...
            if resDict['标识类型'] == 1:
                ...
                contextRegName = contextRegHash([str(None), str(None), str(valDict['innerVal']['user_id'])])
            elif resDict['标识类型'] == 3:
                if 'group_id' in valDict['innerVal'] and 'group_id' in valDict['innerVal']:
                    contextRegName = contextRegHash([
                        str(valDict['innerVal']['host_id']),
            ...
            try:
                ...
                while flagLoop or count > 0:
                    ...
                    if (
                        ...
                        time.sleep(0.5)
                        if ('token' in ChanceCustom.replyContent.contextReg[bot_hash][contextRegName]) and (
                        ...
                        if (
                            resDict['回调函数'] != ''
                            and 'dataQueue' in ChanceCustom.replyContent.contextReg[bot_hash][contextRegName]
                        ):
                            dataQueueCount = len(
                                ChanceCustom.replyContent.contextReg[bot_hash][contextRegName]['dataQueue']
                            )
                            if dataQueueCount > 0:
                                dataQueueTmp = ChanceCustom.replyContent.contextReg[bot_hash][contextRegName][
                                    'dataQueue'
                                ][:dataQueueCount]
                                try:
                                    ChanceCustom.replyContent.contextReg[bot_hash][contextRegName]['dataQueue'] = (
...
def contextRegTryHit(message: str, event_name: str, valDict: dict, bot_hash: str):
    ...
    if event_name == 'group_message':
        ...
        contextRegName_list.append(
            contextRegHash([str(valDict['innerVal']['host_id']), str(valDict['innerVal']['group_id']), str(None)])
        )
        ...
        if (
            ...
            and (
                (
                    ...
                    and (ChanceCustom.replyContent.contextReg[bot_hash][contextRegName]['count'] > 0)
                )
                or (
                    ('flagLoop' in ChanceCustom.replyContent.contextReg[bot_hash][contextRegName])
                    and (ChanceCustom.replyContent.contextReg[bot_hash][contextRegName]['flagLoop'])
                )
            )
        ):
            if 'matchCount' in ChanceCustom.replyContent.contextReg[bot_hash][contextRegName]:
                if ChanceCustom.replyContent.contextReg[bot_hash][contextRegName]['matchCount'] > 0:
                    ...
                    if (
                        ChanceCustom.replyContent.contextReg[bot_hash][contextRegName]['matchCount_count']
                        == ChanceCustom.replyContent.contextReg[bot_hash][contextRegName]['matchCount']
                    ):
                        flagMatch = True
                    ...
                    flagMatch = True
            if ('type' in ChanceCustom.replyContent.contextReg[bot_hash][contextRegName]) and (
                ...
                if 'matchCountOne' in ChanceCustom.replyContent.contextReg[bot_hash][contextRegName]:
                    if ChanceCustom.replyContent.contextReg[bot_hash][contextRegName]['matchCountOne'] > 0:
                        ...
                        contextRegNameOne = contextRegHash([str(None), str(None), str(valDict['innerVal']['user_id'])])
                        if (
                            contextRegNameOne
                            not in ChanceCustom.replyContent.contextReg[bot_hash][contextRegName]['matchCountOne_count']
                        ):
                            ChanceCustom.replyContent.contextReg[bot_hash][contextRegName]['matchCountOne_count'][
                                contextRegNameOne
                            ] = 0
                        ...
                        if (
                            ...
                            == ChanceCustom.replyContent.contextReg[bot_hash][contextRegName]['matchCountOne']
                        ):
                            flagMatch = True
...
def flowOutputFunTemp():
    def flowOutputFun(valDict):
        def flowOutput_f(matched: 're.Match|dict'):
            ...
            msg = resDict['加入文本']
            valDict['innerVal']['plugin_event'].reply(message=msg)
            return res
